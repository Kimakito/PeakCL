...
function ActionBtn({
  ...
  return (
    <a
      ...
      title={label}
      aria-label={label}
      className="group inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-border bg-muted px-2.5 py-2 text-xs font-semibold text-foreground backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-muted sm:flex-none sm:px-3.5"
    >
...
function TrameProspectionPage() {
  ...
  return (
    ...
    >
      ...
      <header
        className="relative z-20 shrink-0 border-b border-border bg-[color-mix(in_oklab,var(--indigo-900)_82%,transparent)] backdrop-blur-2xl"
          ...
          <div className="flex min-w-0 items-center gap-2.5 sm:gap-3.5">
            <div className="relative shrink-0">
              ...
              <img
                src={logo}
                alt="PeakCL"
                className="relative h-9 w-9 rounded-lg object-cover ring-1 ring-border sm:h-11 sm:w-11 sm:rounded-xl"
              />
            </div>
      ...
      <main className="relative z-10 flex min-h-0 flex-1 flex-col p-2 sm:p-3 md:p-5">
        <div className="trame-frame mx-auto flex min-h-0 w-full max-w-[1600px] flex-1 flex-col">
          <div className="trame-canvas relative min-h-0 flex-1">
            {!ready && (
              <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-4 bg-[color-mix(in_oklab,var(--indigo-900)_95%,transparent)]">
            ...
            {ready && (
              <div className="trame-hint pointer-events-none absolute bottom-2 left-1/2 z-20 -translate-x-1/2 px-2 text-center sm:bottom-3">
                <span className="inline-flex flex-wrap items-center justify-center gap-x-1.5 rounded-full border border-border bg-black/40 px-3 py-1.5 text-[10px] font-medium text-muted-foreground backdrop-blur-md sm:text-[11px]">
